  ...
  try {
    ...
    const stageGroups = seasonMatches.filter((m) => m.stage !== 'group').reduce((acc, m) => {
      ...
      acc[key].push({
        ...
        team1Id: m.team1Id,
        team2Id: m.team2Id,
        team1Name: season.teams.find((t) => t.id === m.team1Id)?.name || m.team1Id,
        team2Name: season.teams.find((t) => t.id === m.team2Id)?.name || m.team2Id,
        winnerId: m.winnerId,
        winnerName: season.teams.find((t) => t.id === m.winnerId)?.name || m.winnerId,
        playedAt: m.playedAt,
      });
      return acc;
