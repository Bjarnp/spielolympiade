...
(async () => {
  try {
    ...
    const matches = season.tournaments.flatMap((t) =>
      t.matches.map((match) => normalizeMatch(match as any)),
    );
    ...
    const groupIds = Array.from(
      new Set(groupMatches.flatMap((match) => [match.team1Id, match.team2Id])),
    );
    console.log(
      "group team count",
      groupIds.length,
      groupIds.map((id) => teamNames[id] || id),
    );
    const groupNames = Array.from(
      new Set(groupMatches.map((match) => match.groupName || "A")),
    );
    ...
    const groupCounts = Object.entries(
      groupMatches.reduce(
        ...
        {} as Record<string, { matchCount: number; teamIds: Set<string> }>,
      ),
    ).map(([name, entry]) => ({
      groupName: name,
      matchCount: entry.matchCount,
      teamCount: entry.teamIds.size,
      teams: Array.from(entry.teamIds).map((id) => teamNames[id] || id),
    }));
    ...
    console.log(
      ...
      finalMatches.map((match) => ({
        id: match.id,
        team1Name: teamNames[match.team1Id],
        team2Name: teamNames[match.team2Id],
        winnerName: match.winnerId ? teamNames[match.winnerId] : null,
      })),
    );
