...
import { normalizeMatch, buildGroupKoStandings } from './src/utils/history-core';
...
(async () => {
  try {
    ...
    const teamPlayers = Object.fromEntries(
      season.teams.map((team: any) => [team.id, team.members.map((member: any) => member.user.name)])
    );
    const teamNames = Object.fromEntries(season.teams.map((team: any) => [team.id, team.name]));
    ...
    const stageSummary = matches.reduce((acc: any, match) => {
      const stage = match.stage || 'null';
      if (!acc[stage]) acc[stage] = { count: 0, teamIds: new Set<string>(), pairKeys: new Map<string, number>() };
      ...
      return acc;
    }, {} as Record<string, { count: number; teamIds: Set<string>; pairKeys: Map<string, number> }>);
    ...
    console.log('duplicate counts', JSON.stringify(summary.standings.reduce((acc: any, s) => { acc[s.teamId] = (acc[s.teamId] || 0) + 1; return acc; }, {}), null, 2));
